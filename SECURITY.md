...
- **Nonce replay + idempotent replay** — each mandate nonce is stored in SQLite with a `PRIMARY KEY` constraint. The *first* spend creates exactly one order (atomic `PENDING` reservation); an identical resubmission (same `mandate_id`, same amount, still-valid signature/expiry/policy) returns `ALLOW` with the cached order (`idempotent replay: cached order returned`) without consuming velocity or creating a second order. A resubmission with a different amount, expired mandate, or changed policy falls through to `REJECT` (`nonce already consumed` / policy reason). Replays are thus never double-spent, but they are not all `REJECT`.
